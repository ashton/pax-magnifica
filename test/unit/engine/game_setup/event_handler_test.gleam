...
import engine/game_setup/event_handler
...
pub fn apply_player_joined_adds_player_id_test() {
  ...
  let assert Some(gs) = event_handler.apply(state, event)
...
pub fn apply_data_events_do_not_change_phase_test() {
  ...
  let assert Some(gs1) =
    event_handler.apply(state, SystemTilePlaced("game_1", "tile_1", "hex_1"))
  ...
  let assert Some(gs3b) =
    event_handler.apply(state, PlayerStartingUnitsSetup("game_1", "player_1", []))
  ...
  let assert Some(gs5) =
    event_handler.apply(state, PublicObjectivesRevealed("game_1", []))
