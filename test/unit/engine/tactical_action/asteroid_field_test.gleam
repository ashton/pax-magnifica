...
pub fn ship_cannot_move_into_asteroid_field_test() {
  ...
  let assert Error(_) =
    aggregate.handle_move_units(s, cmd, anomalies.asteroid_field_at(h.origin()))
}
...
pub fn ship_cannot_move_through_asteroid_field_test() {
  ...
  let assert Error(_) =
    aggregate.handle_move_units(
      s,
      cmd,
      anomalies.asteroid_field_at(h.adjacent()),
    )
}
...
pub fn ship_with_antimass_deflectors_can_move_into_asteroid_field_test() {
  ...
  let assert Ok(_) =
    aggregate.handle_move_units(
      s,
      cmd,
      anomalies.asteroid_field_with_deflectors_at(h.origin()),
    )
}
...
pub fn ship_with_antimass_deflectors_can_move_through_asteroid_field_test() {
  ...
  let assert Ok(_) =
    aggregate.handle_move_units(
      s,
      cmd,
      anomalies.asteroid_field_with_deflectors_at(h.adjacent()),
    )
}
